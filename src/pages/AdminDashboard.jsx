...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://127.0.0.1:5001/api/dashboard/admin?role=admin");
  ...
  const handleDeleteUser = async (userId, username) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:5001/api/admin/users/${userId}`, {
        method: 'DELETE'
      });
                ...
                <tbody>
                  {data?.activity_logs?.slice(0, 5).map((log, idx) => (
                    <tr key={idx}>
                      ...
                      <td className={log.status === 'success' ? 'status-success' : 'status-wrong-user'}>
                        {log.status === 'success' ? 'success' : 'Wrong User'}
                      </td>
                      <td style={{ maxWidth: '300px' }}>
                        {log.result_details ? (
                          <div style={{ maxHeight: '120px', overflowY: 'auto', fontSize: '11px', backgroundColor: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '4px' }}>
                            <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                              {(()=>{
                                try { return JSON.stringify(JSON.parse(log.result_details), null, 2) }
                                catch(e) { return log.result_details }
                              })()}
                            </pre>
                          </div>
                        ) : '-'}
                  ...
                  {(!data?.activity_logs || data.activity_logs.length === 0) && (
                    <tr><td colSpan="5" style={{textAlign: 'center', padding: '20px'}}>No recent activity</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                {data?.activity_logs?.map((log, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={{ maxWidth: '400px' }}>
                      {log.result_details ? (
                        <div style={{ maxHeight: '150px', overflowY: 'auto', fontSize: '11px', backgroundColor: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '4px' }}>
                          <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                            {(()=>{
                              try { return JSON.stringify(JSON.parse(log.result_details), null, 2) }
                              catch(e) { return log.result_details }
        ...
        {activeTab === 'Typing Analytics' && (
          <div className="table-card">
            ...
            <div style={{ padding: '20px', backgroundColor: '#2a2d3e', borderRadius: '8px', marginTop: '20px' }}>
              ...
              <p style={{ marginBottom: '10px' }}><strong>Algorithm:</strong> Ensemble (Random Forest, Extra Trees, MLP, GaussianNB) & Statistical Z-Score Matching</p>
              <p style={{ marginBottom: '10px' }}><strong>Decision Process:</strong> Weighted Ensemble Voting System</p>
          ...
          <div className="table-card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
              <div style={{ padding: '20px', backgroundColor: 'rgba(255, 71, 87, 0.1)', border: '1px solid #ff4757', borderRadius: '8px' }}>
                <h3 style={{ color: '#ff4757', marginBottom: '15px' }}>Wrong User Authentication Attempts</h3>
                <h1 style={{ fontSize: '3rem', color: 'white' }}>
                  {data?.activity_logs?.filter(l => l.status === 'blocked' || l.status === 'failed').length || 0}
                </h1>
            ...
            <h3 style={{ marginTop: '30px', marginBottom: '15px', color: '#ff4757' }}>Recent Wrong User Attempts (High Risk)</h3>
            <table className="admin-table">
              <thead>
                <tr>
                  <th>Time</th>
              ...
              <tbody>
                {data?.activity_logs?.filter(l => l.status === 'blocked' || l.status === 'failed').slice(0, 5).map((log, idx) => (
                ...
                {(!data?.activity_logs || data.activity_logs.filter(l => l.status === 'blocked' || l.status === 'failed').length === 0) && (
                  <tr><td colSpan="4" style={{textAlign: 'center', padding: '20px'}}>No wrong user attempts recorded</td></tr>
                )}
              </tbody>
            </table>
          </div>
