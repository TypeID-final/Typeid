...
MODEL_DIR = os.path.join(backend_dir, "models", "keyauth")
...
print(f"[MODEL] Loading models from: {MODEL_DIR}")
try:
    ...
    print(f"[OK] {len(trained_models)} Models loaded successfully.")
...
class AuthService:
    ...
    def get_ml_users(self):
        """Return the list of users the ML model was trained on"""
        try:
            if le is not None:
    ...
    def authenticate_user(self, username, keystroke_features_list):
        ...
        predicted_user, ml_confidence, full_result = self.predict_user_from_keystroke(
            keystroke_features_list
        )
    ...
    def _extract_feature_vector(self, sample):
        ...
        # Parse from string if it's a JSON string
        import json
        ...
        for key in ALL_FEATURES:
            value = sample.get(key, 0.0) if isinstance(sample, dict) else 0.0
            # Handle None values
            if value is None:
                value = 0.0
    ...
    def predict_user_from_keystroke(self, keystroke_features_list):
        ...
        Returns:
            (predicted_user, confidence, full_result_dict)
        ...
        try:
            ...
            features_dict = keystroke_features_list[0] if isinstance(keystroke_features_list, list) else keystroke_features_list

            top_k = 3
            full_vec           = np.array([[features_dict.get(f, 0.0) for f in ALL_FEATURES]])
            ...
            return winner, confidence, full_result
