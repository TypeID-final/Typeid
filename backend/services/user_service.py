...
class UserService:
    ...
    def get_user_registration_id(self, user_id):
        """Get the latest registration_id for a user from user_registration table."""
        conn = self._get_conn()
        try:
            query = """
            ...
            print(f"[ERROR] Error fetching registration_id: {e}")
            return None
        finally:
            conn.close()
