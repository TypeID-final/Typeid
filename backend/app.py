...
def register():
    ...
    try:
        ...
        registration_id = user_service.get_user_registration_id(user_id)

        if not registration_id:
            # Safety: ensure a registration record exists for this user.
            created = user_service.create_user_registration(user_id, password_hash=None)
            if not created:
                return jsonify({
                    'success': False,
            ...
            if not registration_id:
                return jsonify({
                    'success': False,
                    'message': 'Failed to resolve registration record'
                }), 500
...
def login_password():
    ...
    try:
        ...
        # Retrieve stored password hash from user_registration.
        # Role is inferred safely because current schema has no role column here.
        conn = user_service._get_conn()
        try:
            cur = conn.execute("SELECT password FROM user_registration WHERE user_id = ?", (user_id,))
            row = cur.fetchone()
            if not row:
                return jsonify({'access_granted': False, 'message': 'User registration record not found'}), 404
            stored_hash = row[0]
            role = 'admin' if str(username).lower() == 'admin' else 'student'
        finally:
            conn.close()
...
def login_hybrid():
    ...
    try:
        ...
        # Step 2: Check if user is in ML Model
        ml_users = auth_service.get_ml_users()
        
        if username in ml_users:
            # HIGH ACCURACY PATH: Use ML Model (95% accuracy)
            print(f"[OK] User '{username}' found in ML Model - Using ML MODEL (High Accuracy)")
...
@app.route('/api/admin/users/<id>', methods=['DELETE', 'OPTIONS'])
def delete_user(id):
    ...
    try:
        conn = user_service._get_conn()
        try:
            cur = conn.execute("SELECT name FROM user WHERE user_id = ?", (id,))
            row = cur.fetchone()
            if not row:
            ...
            if username.lower() == 'admin':
                return jsonify({'success': False, 'message': 'Cannot delete the master admin user'}), 400
                
            # Delete user and all related records
            conn.execute("DELETE FROM user WHERE user_id = ?", (id,))
            conn.execute("DELETE FROM user_registration WHERE user_id = ?", (id,))
            conn.execute("DELETE FROM biometric_profile WHERE user_id = ?", (id,))
            conn.execute("DELETE FROM login_session WHERE user_id = ?", (id,))
            conn.commit()
...
def ensure_ml_users_exist():
    """Ensure that all users known to the ML model exist in the database so they appear in the Admin Dashboard."""
    try:
        from services.auth_service import AuthService
